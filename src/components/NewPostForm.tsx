...
export const NewPostForm: React.FC<NewPostFormProps> = ({onPostAdded}) => {
  const [errors, setErrors] = useState<{name: boolean, text: boolean}>({name: false, text: false});
  ...
  return (
    <form className={style.box} onSubmit={submitHandler}>
      <div >
        ...
        <input className={style.textfield} type="text" id="username" ref={nameRef} placeholder="Name"/>
        {errors.name && <p>empty name</p>}
      </div>
      <div>
        <label  htmlFor="usertext"></label>
        <textarea className={style.textfield} id="usertext" ref={textRef} placeholder="Message"></textarea>
